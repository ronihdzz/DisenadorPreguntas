...
##############################################################
from CUERPO.DISENO_creador.Creador_Ejecutador.EjecutadorCuestionario_d import Ui_Form
...
from CUERPO.LOGICA_creador.Creador_Ejecutador.EjecutadorCuestionariosVacio import EjecutadorCuestionariosVacios
...
class EjecutadorCuestionario(QtWidgets.QWidget, Ui_Form):
    senalEjecutadorCerrado = pyqtSignal(bool)  # comunicacion con la aplicacionn
    senalCalificacionObtenida=pyqtSignal(float) #cuando saco de calificacion

    def __init__(self,nombreCuestionario="",ordenAleatorio=False,descargados=False,mios=False,oficiales=False,compartidos=False):
        ...
        self.setWindowTitle(f"Nombre del TEST_DELPHIA: {self.nombreCuestionario}")
...
##############################################################################################################################
        ...
        self.ventanas.append(  EjecutadorCuestionariosVacios() )
...
##############################################################################################################################
        ...
        self.listaID_preguntas=list(self.controlABSOLUTO_baseDatos.getAllIds_preguntas())
        if ordenAleatorio: #si el usuario quiere que revolvamos el contenido...
            random.shuffle(self.listaID_preguntas)
...
##############################################################################################################################
    ...
    def terminarCuestionario(self):
        ...
        #Debemos obtener la calificacion de la ultima pregunta
        #ya que al ser la ultima no se queda grabada...
        widgetVisualizando=self.listWidget_panelPreguntas.currentIndex()
        self.listaRespuestas[self.noPreguntas-1] = self.ventanas[widgetVisualizando].getRespuesta()
        self.listaCalificaciones[self.noPreguntas-1] = self.ventanas[widgetVisualizando].calificarRespuesta()
        ...
        self.senalCalificacionObtenida.emit(calificacion)
    ...
    def verRespuestas(self,noPregunta):
        ...
        if self.punteroPregunta != noPregunta:
            ...
            self.bel_punteroPregunta.setText(str(self.punteroPregunta+1))
            #enmarcamos el boton de pregunta,para que se vea que lo seleccionaron...
            self.listBotonesPreguntas[self.punteroPregunta].botonSeleccionado(True)
            #jalamos los datos de la pregunta...
            ...
            #mostramos los datos de la pregunta...
    ...
    def closeEvent(self, event):
        if self.cuestionarioTerminado:
            self.senalEjecutadorCerrado.emit(True)
            event.accept()
        else:
            resultado = QMessageBox.question(self, "Delphi",
                                             "¿Esta seguro de cerrar\n"
                                             f"el test: {self.nombreCuestionario}?",
...
if __name__ == "__main__":
    ...
    application = EjecutadorCuestionario(nombreCuestionario="RoniHernandez",mios=True)
    application.show()
    app.exec()
    #sys.exit(app.exec())
